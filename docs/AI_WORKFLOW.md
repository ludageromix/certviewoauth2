...
23. Prompt test TDD de la rotation de clés et publication JWKS (serveur d'autorisation)

On s'attaque au dernier composant du serveur d'autorisation (authorization-server). On commence par le test d'intégration pour valider la rotation, la publication multi-clés et la non-régression cryptographique des anciens jetons. Consigne : Arrête-toi strictement au rouge après avoir écrit le test et exécuté .\mvnw test. N'écris pas encore l'implémentation. Instructions pour KeyRotationIntegrationTest : Crée KeyRotationIntegrationTest avec @SpringBootTest et @AutoConfigureMockMvc. Écris le scénario de test d'intégration complet en 6 étapes : Étape 1 (JWKS initial) : GET /.well-known/jwks.json renvoie 200 OK et un tableau keys avec 1 seule clé (kid_1). Étape 2 (Premier token) : POST /oauth/token émet un JWT (Jeton A). Décode l'en-tête pour vérifier kid == kid_1. Étape 3 (Rotation) : Effectue un POST /admin/rotate-keys (200 OK). (Note : endpoint de démo non sécurisé, à documenter pour la prod). Étape 4 (JWKS post-rotation & Sécurité) : GET /.well-known/jwks.json renvoie désormais les 2 clés publiques (kid_1 et kid_2). Applique la liste blanche fermée : les seuls champs autorisés par clé sont kty, kid, use, alg, n et e. La présence de tout autre paramètre (ex: secrets d, p, q) fait échouer le test. Étape 5 (Nouveau token) : POST /oauth/token émet un nouveau JWT (Jeton B) dont l'en-tête indique la nouvelle clé active kid == kid_2. Étape 6 (Continuité de service / Non-régression) : Parse le JWKS actualisé, extrait la clé publique correspondant à kid_1, et vérifie cryptographiquement que la signature du Jeton A (émis à l'Étape 2) est toujours valide. Exécute .\mvnw test et montre-moi le résultat rouge !

    Résultat: Test écris avec succès! ajout de l'agent: J'ai ajouté @DirtiesContext(AFTER_CLASS), qui n'était pas dans ta demande. Sans lui, le contexte Spring — mis en cache et partagé entre classes de test — resterait après rotation pour les classes suivantes. 

24. Prompt implémentation de la rotation de clés multi-JWKS (Authorization Server — Passage au Vert)

C'est parti pour le passage au vert ! On implémente la rotation sur l'Authorization Server. Voici la feuille de route, étape par étape : Lier la clé et le kid : Crée un record immuable SigningKey(String kid, RSAPrivateKey privateKey). Mets à jour RsaKeyProvider avec deux méthodes : SigningKey getSigningKey() (la clé active) et List<RSAKey> getAllPublicKeys() (toutes les clés publiques valides). Adapter JwtTokenGenerator et ses tests : Modifie le générateur pour qu'il récupère la clé et son kid en un seul appel via getSigningKey(). Mets à jour le mock de RsaKeyProvider dans ses 24 tests unitaires pour qu'ils repassent immédiatement au vert. Compléter InMemoryRsaKeyProvider : Garde en mémoire l'historique de toutes les clés publiques générées. Ajoute une méthode rotateKeys() qui génère un nouveau couple RSA avec un nouveau kid, le passe en clé active, et ajoute sa clé publique au registre sans effacer les anciennes. Publication du JWKS (JwksController) : Fais renvoyer par GET /.well-known/jwks.json le tableau complet de getAllPublicKeys(). Vérifie bien que seules les clés publiques (kty, kid, use, alg, n, e) sont exposées. Endpoint de rotation (KeyRotationController) : Crée l'endpoint POST /admin/rotate-keys qui déclenche rotateKeys() et confirme le nouveau kid actif. Relance .\mvnw test et montre-moi le résultat une fois que tout repasse au vert !

    Résultat: Implémentation réussie. L'agent me signale un couplage: KeyRotationController dépend de InMemoryRsaKeyProvider, donc de l'implémentation. Je perds ce que je cherchai à implémenter au départ: évoluer vers un keystore ou secrets manager. Il me propose une interface RotatableKeyProvider séparée. Chose que je valide.

25. Prompt de découplage du contrôleur de rotation via une interface dédiée
